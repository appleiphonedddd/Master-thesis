...
import copy
import numpy as np
from flcore.clients.clientinf import clientINF
...
class FedINF(Server):
    ...
    def send_selected_models(self, selected_ids, epoch):
        ...
        for client in [client for client in self.clients if (client.id in selected_ids)]:
            ...
            client.Linear(copy.deepcopy(self.global_model), progress)
    ...
    def train(self):
        ...
        if self.num_new_clients > 0:
            ...
            self.set_new_clients(clientINF)
            print(f"\n-------------Fine tuning round-------------")
