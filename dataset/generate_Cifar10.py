"""
CIFAR-10 federated split generator.

Purpose
-------
- Download/prepare CIFAR-10.
- Merge official train/test into a single pool.
- Partition samples across federated clients (IID / Non-IID).
- Persist per-client train/test splits + a distribution figure.

CLI (current)
-------------
python generate_cifar10.py <iid|noniid> <balance|-> <pat|dir|->
...
Notes
-----
- Images normalized to [-1, 1] via mean/std = 0.5 per channel.
- Full-batch DataLoader loads entire split into RAM for simplicity.
  If you hit OOM, switch to iterative loading and concatenate.
"""
